...
public class Home_Layout : MonoBehaviour
{
    ...
    private List<Toggle> indicators = new();
    private int pageCount = 0;
    private List<Home_Page> pages = new();

    private Dictionary<string, AppObject> appLookup;
    Dictionary<string, List<string>> layoutDictionary = new();

    private string layoutFileName = "com.os6.iconState.json";
    private string layoutFilePath => System_Locations.USER_PREFERENCES + layoutFileName;
    ...
    private void Start()
    {
        indicators.Add(indicatorHolder.transform.GetChild(0).GetComponent<Toggle>());
    ...
    private void Update()
    {
        ...
        if (isLoaded)
            indicators[pageTurner.currentPage - 1].isOn = true;
    }
    ...
    private void LoadAppLayout()
    {
        ...
        for (int i = 0; i < pageCount; i++)
        {
            ...
            GameObject indicator = Instantiate(pageIndicatorPrefab, indicatorHolder.transform);
            indicator.name = (i + 1).ToString();
            indicators.Add(indicator.GetComponent<Toggle>());

            indicator.GetComponent<Toggle>().group = indicatorHolder.GetComponent<ToggleGroup>();
    ...
    private void CreateNewPage()
    {
        ...
        GameObject indicator = Instantiate(pageIndicatorPrefab, indicatorHolder.transform);
        indicator.name = (pageTurner.currentPage).ToString();
        indicators.Add(indicator.GetComponent<Toggle>());
    ...
    private void DestroyEmptyPage()
    {
        foreach (Transform child in pageHolder.transform)
        {
            ...
            {
                GameObject indicator = indicatorHolder.transform.GetChild(indicatorHolder.transform.childCount - 1).gameObject;
                indicators.Remove(indicator.GetComponent<Toggle>());
